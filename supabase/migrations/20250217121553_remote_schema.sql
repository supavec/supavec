...
CREATE OR REPLACE FUNCTION "public"."get_recent_user_activity"() RETURNS TABLE("user_id" "uuid", "signup_date" "date", "api_key_created" boolean, "file_uploaded" boolean)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'auth', 'public'
    AS $$
    SELECT 
        u.id AS user_id,
        DATE(u.created_at AT TIME ZONE 'UTC') AS signup_date,
        CASE 
            WHEN a.id IS NOT NULL THEN true
            ELSE false
        END AS api_key_created,
        CASE 
    ...
    FROM 
        public.profiles u
    LEFT JOIN 
        public.api_keys a ON u.id = a.user_id
    LEFT join
        public.team_memberships tm on u.id = tm.profile_id
    LEFT JOIN 
        public.files f ON tm.team_id = f.team_id
    WHERE 
        u.created_at >= NOW() - INTERVAL '7 days'
    GROUP BY 
        u.id, signup_date, a.id, f.id
    ORDER BY 
        signup_date;
$$;


ALTER FUNCTION "public"."get_recent_user_activity"() OWNER TO "postgres";
  ...
  where
    ...
    and case 
      when filter->>'file_id' is not null and (filter->'file_id'->>'in') is not null then
        metadata->>'file_id' = ANY(ARRAY(SELECT jsonb_array_elements_text(filter->'file_id'->'in')))
      else
        metadata @> filter
    end
...
ALTER TABLE "public"."onboarding_answers" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."onboarding_answers_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY "public"."files"
    ADD CONSTRAINT "files_team_id_fkey" FOREIGN KEY ("team_id") REFERENCES "public"."teams"("id");



ALTER TABLE ONLY "public"."files"
    ADD CONSTRAINT "files_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id");



ALTER TABLE ONLY "public"."onboarding_answers"
    ADD CONSTRAINT "onboarding_answers_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."team_memberships"
    ADD CONSTRAINT "team_memberships_profile_id_fkey" FOREIGN KEY ("profile_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
CREATE POLICY "Allow users to select files if they belong to the team""" ON "public"."files" FOR SELECT USING (("team_id" IN ( SELECT "team_memberships"."team_id"
   FROM "public"."team_memberships"
  WHERE ("team_memberships"."profile_id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "Enable select for users based on id" ON "public"."profiles" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") = "id"));



CREATE POLICY "Enable select for users based on profile_id" ON "public"."team_memberships" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") = "profile_id"));



CREATE POLICY "Enable select for users based on user_id" ON "public"."api_keys" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Enable selet based on profile_id" ON "public"."teams" FOR SELECT USING (("id" IN ( SELECT "team_memberships"."team_id"
   FROM "public"."team_memberships"
  WHERE ("team_memberships"."profile_id" = ( SELECT "auth"."uid"() AS "uid")))));
...
GRANT ALL ON FUNCTION "public"."get_recent_user_activity"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_recent_user_activity"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_recent_user_activity"() TO "service_role";
...
GRANT ALL ON SEQUENCE "public"."onboarding_answers_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."onboarding_answers_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."onboarding_answers_id_seq" TO "service_role";
